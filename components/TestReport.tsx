...
const TestReport: React.FC<TestReportProps> = ({ record, onBack, onDelete, onRetake }) => {
  ...
  // --- PDF GENERATION STATE & REF ---
  const reportRef = useRef<HTMLDivElement>(null);
  const [isDownloading, setIsDownloading] = useState(false);

  // --- 1. LOAD QUESTIONS LOGIC ---
  useEffect(() => {
  ...
  // --- 3. PDF DOWNLOAD ALGORITHM (CDN INJECTION) ---
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      alert("Could not generate PDF. Check internet connection.");
    } finally {
  ...
  const renderSubjectAnalysis = () => {
      ...
      return Object.entries(detailedAnalysis.subjects).map(([subj, data]) => {
          ...
          const weakSkills = Object.entries(typedData.skills).filter(([_, stats]) => {
              const s = stats as { correct: number; total: number; };
              return (s.correct / s.total) < 0.6;
          }).map(([name]) => name);
          
          const strongSkills = Object.entries(typedData.skills).filter(([_, stats]) => {
              const s = stats as { correct: number; total: number; };
  ...
  const getRealityCheck = () => {
      ...
      if (pct >= 80) return { status: "Above Grade Level", desc: `Student demonstrates mastery of ${record.grade} concepts. Ready for advanced topics.`, color: "text-emerald-700 bg-emerald-50 border-emerald-200 dark:text-emerald-300 dark:bg-emerald-900/20 dark:border-emerald-700" };
      if (pct >= 60) return { status: "At Grade Level", desc: `Student meets core expectations for ${record.grade}, but lacks consistency in complex applications.`, color: "text-blue-700 bg-blue-50 border-blue-200 dark:text-blue-300 dark:bg-blue-900/20 dark:border-blue-700" };
      if (pct >= 40) return { status: "Below Grade Level", desc: `Student struggles with ${record.grade} curriculum. Foundational intervention required immediately.`, color: "text-amber-700 bg-amber-50 border-amber-200 dark:text-amber-300 dark:bg-amber-900/20 dark:border-amber-700" };
      return { status: "Critical Risk", desc: `Student is not performing at ${record.grade} level. Significant remedial work needed before progressing.`, color: "text-rose-700 bg-rose-50 border-rose-200 dark:text-rose-300 dark:bg-rose-900/20 dark:border-rose-700" };
  };
  ...
  return (
    <div className="max-w-5xl mx-auto animate-pop pb-32">
        ...
        {/* --- PRINTABLE REPORT CARD (Captured by PDF) --- */}
            ...
            <div className="max-w-5xl mx-auto flex justify-between items-center">
                ...
                <div className="flex gap-3">
                    ...
                    <button 
                        ...
                        className="flex items-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold shadow-lg shadow-emerald-200 dark:shadow-none transition-all hover:-translate-y-1 disabled:opacity-70 disabled:cursor-wait"
                    >
                    ...
                    <button 
                        onClick={onRetake}
                        className="flex items-center gap-2 px-6 py-3 bg-brand-600 hover:bg-brand-500 text-white rounded-xl font-bold shadow-lg shadow-brand-200 transition-all hover:-translate-y-1"
                    >
